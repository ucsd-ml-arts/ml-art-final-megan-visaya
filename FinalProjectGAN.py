...
pic_paths.remove('../fp_data/finalproj_data/cleared_space/.ipynb_checkpoints')
...
for img in pic_paths:
    ...
    resized_img=cropped_img.resize([256,256], PIL.Image.LANCZOS)
        
    temp= np.array(resized_img.convert("RGB"), dtype='float32')
...
im4=Images[601:800]
im5=Images[801:1000]
im6=Images[1001:1200]
im7=Images[1201:1400]
im8=Images[1401:1600]
im9=Images[1601:1800]
im10=Images[1800:len(Images)]


# In[50]:


ImageList=[im1,im2,im3,im4,im5,im6,im7,im8,im9,im10]
...
while(model.GAN.steps < 100000):
    #if model.GAN.steps % 10000:
     #   imlist_idx=0;
    #if model.GAN.steps % 1000 == 0:
     #   Images=ImageList[imlist_idx]
      #  imlist_idx+=1;
    ...
    if model.GAN.steps % 1000 == 0:
        print("\n\n\n\nRound " + str(model.GAN.steps) + ":")
        model.eval2(int(model.GAN.steps / 1000) + 20)
    print(time.time())
